...
\midrule
17/07/2026 & 1.0 & Roberto Capone; Guglielmo Cattaneo & Initial complete release aligned with the final implementation contained in the project repository and Requirements Analysis and Specification version 1.0.\\
\bottomrule
\end{tabularx}
\end{table}
...
The design describes the final implementation contained in the project repository. It therefore serves both as an architectural specification and as a traceability reference for reviewing the submitted source code.
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  comp/.style={draw, rounded corners=3pt, align=center, minimum width=3.15cm, minimum height=0.95cm, fill=LightBlue, line width=0.7pt},
  ext/.style={draw, rounded corners=2pt, align=center, minimum width=3.0cm, minimum height=0.9cm, fill=LightGray, line width=0.7pt},
  store/.style={draw, cylinder, shape border rotate=90, aspect=0.22, align=center, minimum width=3.2cm, minimum height=1.0cm, fill=LightGreen, line width=0.7pt},
  dep/.style={-{Latex[length=2.2mm]}, line width=0.8pt},
  inherit/.style={-{Triangle[open,length=3mm,width=3mm]}, line width=0.9pt},
  runtime/.style={-{Latex[length=2.2mm]}, dashed, line width=0.75pt},
  layer/.style={draw=PolimiBlue!70, rounded corners=4pt, dashed, inner sep=5mm},
  lab/.style={font=\scriptsize,fill=white,inner sep=1.2pt},
  font=\small
]
\node[ext] (launcher) at (0,3.5) {Operator / \code{start.sh}};
\node[comp] (cli) at (4.2,3.5) {Application entry point\\\code{cli.py}};
\node[comp] (es) at (4.2,1.3) {Evolutionary controller\\\code{ESGenerator}};
\node[comp] (random) at (0,1.3) {Random baseline policy\\\code{RandomGenerator}};
\node[comp] (config) at (8.4,3.5) {Configuration\\\code{config.py}};
\node[comp] (geometry) at (8.4,1.3) {Domain services\\\code{geometry.py}};
\node[comp] (model) at (12.6,3.5) {Candidate model\\\code{Individual}};
\node[comp] (testcase) at (12.6,1.3) {Simulation adapter\\\code{TestCase}};
\node[comp] (plot) at (4.2,-1.2) {Failure visualisation\\\code{plot\_official.py}};
\node[store] (fs) at (8.4,-1.2) {Filesystem\\checkpoint, failures, delivery};
\node[ext] (aerialist) at (12.6,-1.2) {Aerialist library};
\node[ext] (sim) at (16.5,-1.2) {PX4-Avoidance /\\Gazebo SITL};

\begin{scope}[on background layer]
\node[layer,fit=(cli)(es)(random),label={[PolimiBlue,font=\bfseries\scriptsize]above:Application and search layer}] {};
\node[layer,fit=(config)(geometry)(model),label={[PolimiBlue,font=\bfseries\scriptsize]above:Domain and configuration layer}] {};
\node[layer,fit=(testcase)(plot)(fs)(aerialist)(sim),label={[PolimiBlue,font=\bfseries\scriptsize]below:Infrastructure and presentation layer}] {};
\end{scope}

\draw[dep] (launcher) -- (cli);
\draw[dep] (cli) -- (es);
\draw[inherit] (random) -- (es);
\draw[dep] (config) -- (es);
\draw[dep] (config) -- (geometry);
\draw[dep] (es) -- (geometry);
\draw[dep] (es) -- (model);
\draw[dep] (es) -- (testcase);
\draw[runtime] (es) -- node[lab,sloped,above]{lazy call} (plot);
\draw[dep] (es) -- (fs);
\draw[dep] (model) -- (geometry);
\draw[dep] (model) -- (testcase);
\draw[dep] (testcase) -- (aerialist);
\draw[dep] (plot) -- (fs);
\draw[dep] (aerialist) -- (sim);
\end{tikzpicture}%
}
\caption{Logical component view with explicit application, domain, and infrastructure responsibilities.}
...
\begin{tikzpicture}[
  module/.style={draw, rounded corners=3pt, minimum width=3.1cm, minimum height=0.88cm, align=center, fill=LightBlue, line width=0.7pt},
  external/.style={draw, rounded corners=2pt, minimum width=3.1cm, minimum height=0.88cm, align=center, fill=LightGray, line width=0.7pt},
  dep/.style={-{Latex[length=2mm]}, line width=0.75pt},
  lazy/.style={-{Latex[length=2mm]}, dashed, line width=0.7pt},
  lab/.style={font=\scriptsize,fill=white,inner sep=1.2pt},
  font=\small
]
\node[module] (cli) at (6.0,4.8) {cli.py};
\node[module] (es) at (3.0,2.5) {es\_generator.py};
\node[module] (random) at (9.0,2.5) {random\_generator.py};
\node[module] (geometry) at (0,0) {geometry.py};
\node[module] (models) at (4.0,0) {models.py};
\node[module] (testcase) at (8.0,0) {testcase.py};
\node[module] (plot) at (12.0,0) {plot\_official.py};
\node[module] (config) at (0,-2.7) {config.py};
\node[external] (aerialist) at (6.0,-2.7) {Aerialist};
\node[external] (numeric) at (12.0,-2.7) {NumPy / Matplotlib /\\PyYAML / Shapely};

\draw[dep] (cli) -- (es);
\draw[dep] (cli) -- (random);
\draw[dep] (random) -- node[lab,sloped,above]{inherits} (es);
\draw[dep] (random) -- (geometry);
\draw[dep] (random) -- (models);
\draw[dep] (es) -- (geometry);
\draw[dep] (es) -- (models);
\draw[dep] (es) -- (testcase);
\draw[dep] (es) -- (config);
\draw[lazy] (es) to[bend left=8] node[lab,sloped,above]{runtime import} (plot);
\draw[dep] (models) -- (geometry);
\draw[dep] (models) -- (testcase);
\draw[dep] (geometry) -- (config);
\draw[dep] (geometry) -- (aerialist);
\draw[dep] (geometry) -- (numeric);
\draw[dep] (testcase) -- (aerialist);
\draw[dep] (plot) -- (numeric);
\draw[lazy] (plot) to[bend left=16] node[lab,sloped,below]{fallback reader} (aerialist);
\end{tikzpicture}%
}
\caption{Static Python-module dependencies. Dashed edges denote imports performed only on a runtime path.}
\label{fig:modules}
\end{figure}
...
\centering
\begin{tikzpicture}[
  mainclass/.style={draw, rounded corners=2pt, fill=LightBlue, text width=13.4cm,
                    align=left, inner sep=3mm, line width=0.85pt, font=\scriptsize},
  classbox/.style={draw, rounded corners=2pt, fill=LightBlue, text width=4.05cm,
                   align=left, inner sep=2.5mm, minimum height=3.4cm,
                   line width=0.75pt, font=\scriptsize},
  service/.style={draw, rounded corners=2pt, fill=LightGray, text width=6.0cm,
                  align=left, inner sep=2.5mm, minimum height=1.7cm,
                  line width=0.75pt, font=\scriptsize},
  inherit/.style={-{Triangle[open,length=3mm,width=3mm]}, line width=0.9pt},
  relation/.style={-{Latex[length=2.1mm]}, line width=0.78pt},
  dependency/.style={-{Latex[length=2.0mm]}, dashed, line width=0.72pt},
  lab/.style={font=\scriptsize, fill=white, inner sep=1.1pt, align=center}
]
\node[mainclass] (es) at (0,5.0) {
  \centering\textbf{ESGenerator}\\[-1mm]
  \raggedright
  \textbf{State:} case study; budget counters; archive and parent; sigma;
  generation and stagnation counters; output directory; pending candidate;
  resumable seed state.\\[1mm]
  \textbf{Public operations:} \code{generate(budget)},
  \code{save\_checkpoint()}, \code{write\_final\_selection()}.\\[1mm]
  \textbf{Principal internal operations:} evaluation; route bootstrap;
  checkpoint restoration; seed evaluation; offspring creation; sigma adaptation;
  restart and intensification; archive update; failure persistence; duplicate
  detection; delivery selection.
};
\node[classbox] (random) at (-4.85,0.35) {
  \centering\textbf{RandomGenerator}\\[-1mm]
  \raggedright
  Inherits evaluation, checkpoint, archive, and delivery behaviour.\\[1mm]
  \textbf{Overrides:}\\
  \code{\_make\_child(parent,sigma)}\\
  \code{\_restart\_individual()}\\[1mm]
  Both return independent random samples.
};
\node[classbox] (ind) at (0,0.35) {
  \centering\textbf{Individual}\\[-1mm]
  \raggedright
  \textbf{Attributes:} genes; fitness; minimum distance; completion flag;
  temporary test case; persistent failure path; average point; average duration;
  resampled trajectory.\\[1mm]
  \textbf{Operation:}\\
  \code{obstacles() : Obstacle[ ]}
};
\node[classbox] (tc) at (4.85,0.35) {
  \centering\textbf{TestCase}\\[-1mm]
  \raggedright
  \textbf{Attributes:} copied Aerialist test; trajectory; log file.\\[1mm]
  \textbf{Operations:}\\
  \code{execute() : Trajectory}\\
  \code{get\_distances() : float[ ]}\\
  \code{save\_yaml(path)}
};
\node[service] (domain) at (-3.25,-4.25) {
  \textbf{Module-level domain services}\\
  \code{geometry.py}: legal fitting, overlap checks, route handling, sampling,
  mutation, DTW, scoring, trap-gene construction, and repair.\\
  \code{config.py}: official bounds and environment-driven parameters.
};
\node[service] (external) at (3.25,-4.25) {
  \textbf{External execution types}\\
  \code{AerialistTest}, \code{Obstacle}, \code{Trajectory}, and the selected
  \code{LocalAgent}, \code{DockerAgent}, or \code{K8SAgent}.
};

\draw[inherit] (random.north) -- ++(0,0.65) -| (es.south west);
\node[lab] at (-5.35,2.65) {inherits};
\draw[relation] (es.south) -- node[lab,right]{parent and archive} (ind.north);
\draw[relation] (es.south east) -- node[lab,right,pos=0.62]{creates} (tc.north);
\draw[dependency] (random.south) -- node[lab,left]{sampling policy} (domain.north west);
\draw[dependency] (ind.south) -- node[lab,left]{conversion services} (domain.north east);
\draw[dependency] (tc.south) -- node[lab,right]{adapts} (external.north east);
\end{tikzpicture}
\caption{Principal class model in portrait layout, synchronized with
\texttt{docs/uml/class\_diagram.mmd}. Utility functions and constants remain
module-level services rather than artificial classes.}
\label{fig:classes}
\end{figure}
...
The class, sequence, and execution-flow figures are synchronized with the repository-maintained Mermaid sources in \texttt{Deliverable\_3\_Implementation/docs/uml/}. The TikZ versions below are used to keep this document self-contained and printable.
...
\centering
\begin{tikzpicture}[
  participant/.style={draw, rounded corners=2pt, fill=LightBlue,
                      minimum width=2.8cm, minimum height=0.78cm,
                      align=center, line width=0.7pt, font=\small},
  ...
  framelabel/.style={anchor=north west, font=\scriptsize\bfseries,
                    fill=white, inner sep=1.2pt},
  note/.style={draw, fill=LightGray, align=center, text width=6.4cm,
               font=\scriptsize, inner sep=2mm},
  txt/.style={font=\scriptsize, fill=white, inner sep=1.0pt, align=center}
]
\def\XUser{0}\def\XCLI{4.0}\def\XGen{8.0}\def\XFS{12.0}
\node[participant] at (\XUser,0) {User};
\node[participant] at (\XCLI,0) {cli.py};
\node[participant] at (\XGen,0) {ESGenerator};
\node[participant] at (\XFS,0) {Filesystem};
\foreach \x in {\XUser,\XCLI,\XGen,\XFS}
  \draw[dashed,gray] (\x,-0.4)--(\x,-12.8);

\draw[msg] (\XUser,-1.0)--node[txt,above]{generate mission.yaml budget}(\XCLI,-1.0);
\draw[msg] (\XCLI,-1.8)--node[txt,above]{construct generator}(\XGen,-1.8);
\draw[msg] (\XGen,-2.6)--node[txt,above]{load mission and nominal ULog}(\XFS,-2.6);
\draw[ret] (\XFS,-3.3)--node[txt,above]{route or unavailable}(\XGen,-3.3);
\draw[msg] (\XCLI,-4.1)--node[txt,above]{generate(budget)}(\XGen,-4.1);

\node[frame,fit={(6.9,-4.65)(13.15,-8.2)},inner sep=2mm] (altframe) {};
\node[framelabel] at (altframe.north west) {alt resumable checkpoint / clean start};
\draw[msg] (\XGen,-5.5)--node[txt,above]{read checkpoint.json}(\XFS,-5.5);
\draw[ret] (\XFS,-6.25)--node[txt,above]{parent, archive, sigma, pending, route}(\XGen,-6.25);
\node[note] at (8.9,-7.35) {On a clean start, bootstrap the route when required,
prepare \code{INIT\_SEEDS}, and checkpoint the resumable seed state.};

\node[frame,fit={(6.85,-8.65)(13.15,-12.25)},inner sep=2mm] (seedframe) {};
\node[framelabel] at (seedframe.north west) {opt seeding active};
\node[note] at (9.1,-9.55) {Evaluate each remaining seed with a checkpoint before
and after execution.};
\draw[msg] (\XGen,-10.55)--node[txt,above]{save pending seed}(\XFS,-10.55);
\draw[msg] (\XGen,-11.35)--node[txt,above]{save completed seed state}(\XFS,-11.35);
\node[note] at (9.2,-12.05) {Select the best evaluated seed as the initial parent.};
\end{tikzpicture}
\caption{Runtime sequence, panel 1: invocation, route acquisition,
checkpoint-aware initialization, and resumable seeding.}
\label{fig:sequence-execution}
\end{figure}
...
\def\XGen{0}\def\XTC{4.0}\def\XAgent{8.0}\def\XSim{12.0}
\node[participant] at (\XGen,0) {ESGenerator};
\node[participant] at (\XTC,0) {TestCase};
\node[participant] at (\XAgent,0) {Aerialist Agent};
\node[participant] at (\XSim,0) {PX4 / Gazebo};
\foreach \x in {\XGen,\XTC,\XAgent,\XSim}
  \draw[dashed,gray] (\x,-0.4)--(\x,-12.8);

\node[note] at (2.6,-1.05) {Create a mutated child, or reconstruct the pending genes
after a process crash.};
\draw[msg] (\XGen,-2.15)--node[txt,above]{construct TestCase}(\XTC,-2.15);
\draw[msg] (\XGen,-3.0)--node[txt,above]{execute()}(\XTC,-3.0);
\draw[msg] (\XTC,-3.85)--node[txt,above]{run()}(\XAgent,-3.85);
\draw[msg] (\XAgent,-4.7)--node[txt,above]{start mission with obstacles}(\XSim,-4.7);
\draw[ret] (\XSim,-5.55)--node[txt,above]{trajectory recorded in ULog}(\XAgent,-5.55);
\draw[ret] (\XAgent,-6.4)--node[txt,above]{trajectory and log file}(\XTC,-6.4);
\draw[ret] (\XTC,-7.25)--node[txt,above]{trajectory}(\XGen,-7.25);
\draw[msg] (\XGen,-8.1)--node[txt,above]{get distances}(\XTC,-8.1);
\draw[ret] (\XTC,-8.95)--node[txt,above]{minimum obstacle distances}(\XGen,-8.95);

\node[frame,fit={(-1.25,-9.45)(5.35,-11.65)},inner sep=2mm] (confirmframe) {};
\node[framelabel] at (confirmframe.north west) {opt critical distance below threshold};
\draw[msg] (\XGen,-10.35)--node[txt,above]{execute confirmation run}(\XTC,-10.35);
\draw[ret] (\XTC,-11.15)--node[txt,above]{additional trajectory and distance}(\XGen,-11.15);
\node[note] at (3.0,-12.15) {Aggregate minimum distance, average competition point,
average flight time, completion, and average resampled trajectory.};
\end{tikzpicture}
\caption{Runtime sequence, panel 2: candidate execution through Aerialist,
telemetry return, distance measurement, and optional anti-noise confirmation.}
...
\begin{tikzpicture}[
  ...
  framelabel/.style={anchor=north west, font=\scriptsize\bfseries,
                    fill=white, inner sep=1.2pt},
  note/.style={draw, fill=LightGray, align=center, text width=6.5cm,
               font=\scriptsize, inner sep=2mm},
  txt/.style={font=\scriptsize, fill=white, inner sep=1.0pt, align=center}
]
\def\XUser{0}\def\XCLI{4.0}\def\XGen{8.0}\def\XFS{12.0}
...
\node[participant] at (\XFS,0) {Filesystem};
\foreach \x in {\XUser,\XCLI,\XGen,\XFS}
  \draw[dashed,gray] (\x,-0.4)--(\x,-13.0);

\node[note] at (8.5,-1.15) {Persist every valid failure, compare its trajectory by
DTW, and retain only the most severe representative of an equivalent basin.};
\draw[msg] (\XGen,-2.35)--node[txt,above]{write YAML and copy ULog}(\XFS,-2.35);
\draw[msg] (\XGen,-3.15)--node[txt,above]{write failure plot when available}(\XFS,-3.15);
\node[note] at (8.5,-4.15) {Update the parent and stagnation counter, adapt sigma,
and restart or intensify when the stagnation threshold is reached.};
\draw[msg] (\XGen,-5.25)--node[txt,above]{save completed checkpoint}(\XFS,-5.25);

\node[frame,fit={(5.8,-5.8)(13.3,-8.15)},inner sep=2mm] (recoveryframe) {};
\node[framelabel] at (recoveryframe.north west) {alt external process failure};
\node[note] at (9.5,-6.75) {\code{start.sh} restarts the container. The pending
candidate is replayed from the beginning, while prior artifacts remain mounted.};
\draw[ret] (\XFS,-7.65)--node[txt,above]{checkpoint and stored failures}(\XGen,-7.65);

\draw[msg] (\XCLI,-9.0)--node[txt,above]{write final selection}(\XGen,-9.0);
\node[note] at (8.6,-9.95) {Sort by delivery score and greedily enforce the configured
DTW separation.};
\draw[msg] (\XGen,-11.0)--node[txt,above]{copy rankNN artifacts to consegna}(\XFS,-11.0);
\draw[ret] (\XGen,-11.85)--node[txt,above]{delivered test count}(\XCLI,-11.85);
\draw[ret] (\XCLI,-12.65)--node[txt,above]{final delivery path}(\XUser,-12.65);
\end{tikzpicture}
\caption{Runtime sequence, panel 3: persistent failure handling, archive update,
recovery semantics, and final ranked delivery. Together with
Figures~\ref{fig:sequence-execution} and~\ref{fig:sequence-simulation}, this
is synchronized with \texttt{docs/uml/sequence\_diagram.mmd}.}
\label{fig:sequence-delivery}
\end{figure}


\subsection{Detailed Execution Flow}
Figures~\ref{fig:executionflow-init}, \ref{fig:executionflow-loop}, and~\ref{fig:executionflow-delivery} provide the portrait document rendering of the repository flowchart. Together they cover clean initialization, checkpoint restoration, seeding, the complete evolutionary iteration, stagnation handling, and delivery construction.
...
\centering
\begin{tikzpicture}[
  startstop/.style={draw, rounded corners=7pt, align=center, text width=4.0cm,
                   minimum height=0.78cm, fill=LightGray, line width=0.78pt},
  process/.style={draw, rounded corners=3pt, align=center, text width=4.6cm,
                 minimum height=0.82cm, fill=LightBlue, line width=0.72pt},
  decision/.style={draw, diamond, aspect=2.35, align=center, text width=2.6cm,
                  fill=LightGray, line width=0.72pt, inner sep=1.3pt},
  arr/.style={-{Latex[length=2.2mm]}, line width=0.78pt},
  lab/.style={font=\scriptsize, fill=white, inner sep=1.0pt, align=center},
  font=\small
]
\node[startstop] (start) at (0,0) {CLI invocation with mission and budget};
\node[process] (init) at (0,-1.65) {Construct \code{ESGenerator}; load the mission and
nominal route; tune positional mutation scales};
\node[process] (generate) at (0,-3.25) {Enter \code{generate(budget)}};
\node[decision] (checkpoint) at (0,-4.9) {valid checkpoint?};
\node[process] (load) at (-3.9,-7.1) {Restore budget, parent, archive, sigma,
pending test, seed state, route, and destination};
\node[process] (clean) at (3.9,-7.1) {Clean start: load or bootstrap the route,
prepare \code{INIT\_SEEDS}, and save resumable seed state};
\node[decision] (seeding) at (0,-9.45) {seeding active?};
\node[process] (seed) at (-3.9,-11.7) {Evaluate each remaining seed; checkpoint
before and after each evaluation};
\node[process] (parent) at (3.9,-11.7) {Use the restored or best evaluated parent
and close the seeding phase};
\node[decision] (loop) at (0,-14.0) {budget remains?};
\node[startstop] (rank) at (-3.9,-16.1) {No: return the archive ordered by the
delivery score};
\node[startstop] (iterate) at (3.9,-16.1) {Yes: enter an evolutionary iteration};

\draw[arr] (start)--(init);
\draw[arr] (init)--(generate);
\draw[arr] (generate)--(checkpoint);
\draw[arr] (checkpoint)--node[lab,sloped,above]{yes}(load);
\draw[arr] (checkpoint)--node[lab,sloped,above]{no}(clean);
\draw[arr] (load.south east)--(seeding.north west);
\draw[arr] (clean.south west)--(seeding.north east);
\draw[arr] (seeding)--node[lab,sloped,above]{yes}(seed);
\draw[arr] (seeding)--node[lab,sloped,above]{no}(parent);
\draw[arr] (seed.south east)--(loop.north west);
\draw[arr] (parent.south west)--(loop.north east);
\draw[arr] (loop)--node[lab,sloped,above]{no}(rank);
\draw[arr] (loop)--node[lab,sloped,above]{yes}(iterate);
\end{tikzpicture}
\caption{Execution-flow panel 1: construction, checkpoint restoration,
route acquisition or bootstrap, resumable seeding, and loop entry.}
\label{fig:executionflow-init}
\end{figure}
...
\centering
\begin{tikzpicture}[
  startstop/.style={draw, rounded corners=7pt, align=center, text width=4.3cm,
                   minimum height=0.82cm, fill=LightGray, line width=0.78pt},
  process/.style={draw, rounded corners=3pt, align=center, text width=5.0cm,
                 minimum height=0.88cm, fill=LightBlue, line width=0.72pt},
  store/.style={draw, rounded corners=3pt, align=center, text width=5.0cm,
               minimum height=0.88cm, fill=LightGreen, line width=0.72pt},
  decision/.style={draw, diamond, aspect=2.35, align=center, text width=2.8cm,
                  fill=LightGray, line width=0.72pt, inner sep=1.4pt},
  arr/.style={-{Latex[length=2.2mm]}, line width=0.78pt},
  lab/.style={font=\scriptsize, fill=white, inner sep=1.1pt, align=center},
  node distance=8mm and 15mm,
  font=\small
]
\node[startstop] (enter) {Evolutionary iteration};
\node[decision,below=of enter] (pending) {pending test from crash?};
\node[process,below left=9mm and 16mm of pending] (reuse) {Reconstruct the stored genes
and rerun the candidate from the beginning};
\node[process,below right=9mm and 16mm of pending] (child) {Create one child by mutation
and optional obstacle addition or removal};
\node[store,below=24mm of pending] (savep) {Atomically save a checkpoint with
\code{pending = candidate genes}};
\node[process,below=of savep] (evaluate) {Evaluate the child using retries, timeout,
repetitions, and optional confirmation};
\node[decision,below=of evaluate] (improved) {child improves parent?};
\node[process,below left=9mm and 16mm of improved] (accept) {Accept the child;
reset stagnation};
\node[process,below right=9mm and 16mm of improved] (reject) {Keep the parent;
increment stagnation};
\node[store,below=24mm of improved] (archive) {Persist every valid failure and apply the
DTW archive replacement / admission policy};
\node[decision,below=of archive] (redundant) {redundant failure?};
\node[process,below left=9mm and 16mm of redundant] (extra) {Increment stagnation once
more to leave the exhausted trajectory basin};
\node[process,below right=9mm and 16mm of redundant] (sigma) {Adapt sigma with the
Rechenberg one-fifth success rule};
\node[startstop,below=24mm of redundant] (continue) {Proceed to stagnation recovery and
generation completion};

\draw[arr] (enter)--(pending);
\draw[arr] (pending)--node[lab,sloped,above]{yes}(reuse);
\draw[arr] (pending)--node[lab,sloped,above]{no}(child);
\draw[arr] (reuse.south east)--(savep.north west);
\draw[arr] (child.south west)--(savep.north east);
\draw[arr] (savep)--(evaluate);
\draw[arr] (evaluate)--(improved);
\draw[arr] (improved)--node[lab,sloped,above]{yes}(accept);
\draw[arr] (improved)--node[lab,sloped,above]{no}(reject);
\draw[arr] (accept.south east)--(archive.north west);
\draw[arr] (reject.south west)--(archive.north east);
\draw[arr] (archive)--(redundant);
\draw[arr] (redundant)--node[lab,sloped,above]{yes}(extra);
\draw[arr] (redundant)--node[lab,sloped,above]{no}(sigma);
\draw[arr] (extra.south east)--(continue.north west);
\draw[arr] (sigma.south west)--(continue.north east);
\end{tikzpicture}
\caption{Execution-flow panel 2: candidate reconstruction or creation,
evaluation, parent selection, persistent failure handling, archive update,
and sigma adaptation.}
\label{fig:executionflow-loop}
\end{figure}
...
\begin{tikzpicture}[
  startstop/.style={draw, rounded corners=7pt, align=center, text width=4.0cm,
                   minimum height=0.78cm, fill=LightGray, line width=0.78pt},
  process/.style={draw, rounded corners=3pt, align=center, text width=4.4cm,
                 minimum height=0.82cm, fill=LightBlue, line width=0.72pt},
  store/.style={draw, rounded corners=3pt, align=center, text width=4.4cm,
               minimum height=0.82cm, fill=LightGreen, line width=0.72pt},
  decision/.style={draw, diamond, aspect=2.35, align=center, text width=2.6cm,
                  fill=LightGray, line width=0.72pt, inner sep=1.3pt},
  arr/.style={-{Latex[length=2.2mm]}, line width=0.78pt},
  lab/.style={font=\scriptsize, fill=white, inner sep=1.0pt, align=center},
...
\node[startstop] (enter) at (0,0) {Stagnation recovery and generation completion};
\node[decision] (stag) at (0,-1.9) {stagnation threshold reached?};
\node[decision] (covered) at (-2.7,-4.15) {all route buckets covered?};
\node[process] (normal) at (4.15,-4.15) {No restart: preserve the parent, sigma,
and counters};
\node[process] (diversify) at (-4.65,-6.55) {Restart in an uncovered route bucket and
select the least-overlapping candidate};
\node[process] (intensify) at (0.55,-6.55) {When selected, intensify from the most severe
failure and optionally add a trap gene};
\node[process] (reset) at (-2.05,-8.85) {After a restart, reset sigma, stagnation,
and the recent success history};
\node[store] (saveg) at (0,-10.9) {Atomically save the completed generation checkpoint};
\node[decision] (budget) at (0,-12.8) {budget remains?};
\node[startstop] (next) at (-4.2,-14.85) {Return to the next evolutionary iteration};
\node[process] (rank) at (4.2,-14.85) {Sort the representative archive and greedily
enforce DTW separation};
\node[store] (copy) at (4.2,-17.05) {Copy selected YAML, ULog, and plot artifacts from
\code{all\_fails} to \code{consegna} as \code{rankNN}};
\node[startstop] (end) at (4.2,-19.0) {Log the delivery path and number of delivered tests};

\draw[arr] (enter)--(stag);
\draw[arr] (stag)--node[lab,sloped,above]{yes}(covered);
\draw[arr] (stag)--node[lab,sloped,above]{no}(normal);
\draw[arr] (covered)--node[lab,sloped,above]{no}(diversify);
\draw[arr] (covered)--node[lab,sloped,above]{yes}(intensify);
\draw[arr] (diversify.south east)--(reset.north west);
\draw[arr] (intensify.south west)--(reset.north east);
\draw[arr] (reset)--(saveg);
\draw[arr] (normal.east) -- ++(0.65,0) |- (saveg.east);
\draw[arr] (saveg)--(budget);
\draw[arr] (budget)--node[lab,sloped,above]{yes}(next);
\draw[arr] (budget)--node[lab,sloped,above]{no}(rank);
\draw[arr] (rank)--(copy);
\draw[arr] (copy)--(end);
\end{tikzpicture}
\caption{Execution-flow panel 3: stagnation recovery, restart policy,
checkpoint completion, loop continuation, and final delivery. Together with
Figures~\ref{fig:executionflow-init} and~\ref{fig:executionflow-loop}, this is
synchronized with \texttt{docs/uml/execution\_flow.mmd}.}
\label{fig:executionflow-delivery}
\end{figure}
...
\centering
\begin{tikzpicture}[
  host/.style={draw=PolimiBlue, rounded corners=4pt, align=center, text width=6.0cm,
               minimum height=1.0cm, fill=LightBlue, line width=0.9pt},
  nodebox/.style={draw, rounded corners=3pt, align=center, text width=6.4cm,
                 minimum height=1.05cm, fill=LightBlue, line width=0.75pt},
  store/.style={draw, rounded corners=3pt, align=center, text width=4.7cm,
               minimum height=1.15cm, fill=LightGreen, line width=0.75pt},
  ext/.style={draw, rounded corners=3pt, align=center, text width=4.7cm,
             minimum height=1.0cm, fill=LightGray, line width=0.75pt},
  note/.style={font=\scriptsize, align=center, text width=4.5cm},
  arr/.style={-{Latex[length=2.2mm]}, line width=0.8pt},
  mount/.style={-{Latex[length=2.2mm]}, dashed, line width=0.75pt},
  lab/.style={font=\scriptsize, fill=white, inner sep=1.1pt, align=center},
  font=\small
]
\node[host] (host) at (0,4.7) {Linux host: Docker Engine, shell, and \code{start.sh}};
\node[store] (repo) at (-4.3,2.0) {Repository mount\\source code and case studies};
\node[store] (out) at (4.3,2.0) {Persistent output mount\\checkpoint, logs,
\code{all\_fails}, and \code{consegna}};
\node[note] at (-4.3,0.85) {read-only logical input; copied into the runtime};
\node[nodebox] (container) at (0,-0.65) {\code{skhatiri/aerialist} container\\
Python generator, Aerialist runtime, and campaign process};
\node[ext] (px4) at (-3.6,-4.7) {PX4-Avoidance SITL\\software under test};
\node[ext] (gazebo) at (3.6,-4.7) {Gazebo SITL\\simulation environment};

\node[note,text width=3.2cm] at (1.9,3.45) {\code{docker run}; automatic restart};
\draw[arr] (host)--(container);
\draw[mount] (repo.west) -- ++(-0.65,0) |- (container.north west);
\draw[mount] (container.north east) -- node[lab,sloped,above]{persistent read/write mount} (out.south east);
\draw[arr] (container.south west)--node[lab,sloped,above]{launches}(px4.north);
\draw[arr] (container.south east)--node[lab,sloped,above]{launches}(gazebo.north);
\draw[arr] (px4.east) to[bend left=16] node[lab,above]{control and telemetry}(gazebo.west);
\draw[arr] (gazebo.west) to[bend left=16] node[lab,below]{sensor and world state}(px4.east);
\end{tikzpicture}
\caption{Reference Docker deployment and the two host-mounted persistence boundaries.}
...
\item R. Capone and G. Cattaneo, \emph{Requirements Analysis and Specification Document -- Search-Based Test Generation for Autonomous UAV Obstacle Avoidance Using a $(1+1)$-Evolution Strategy}, version 1.0, 17 July 2026.

\item M. Camilli, \emph{Software Engineering (for Automation) -- Project Guideline}, Politecnico di Milano, A.Y. 2024--2025.

\item R. Capone and G. Cattaneo, \emph{Search-Based Test Generation for Autonomous UAV Obstacle Avoidance Using a $(1+1)$-Evolution Strategy}, Software Engineering Project Repository, 2026. Available: \url{https://github.com/gulicatta/SE4A_RobertoCapone_GuglielmoCattaneo}.
...
\item I. Rechenberg, \emph{Evolutionsstrategie: Optimierung technischer Systeme nach Prinzipien der biologischen Evolution}. Frommann-Holzboog, 1973.
\end{enumerate}
...
\AddToShipoutPictureFG*{%
  \AtPageUpperLeft{%
    \raisebox{-1.85cm}[0pt][0pt]{%
      \hspace{2.45cm}\includegraphics[width=2.70cm]{polimi_logo.png}%
    }%
  }%
}
